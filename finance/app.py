...
def index():
    ...
    rows = db.execute("SELECT * FROM users WHERE username = :id", id=session["user_id"])
...
def buy():
    ...
    # if request method is POST
        ...
        # query users table for row with this username and stock symbol:
        row = db.execute("SELECT * FROM users WHERE username = :id AND symbol = :symbol",
                         id=session["user_id"], symbol=symbol)

        # if row doesn't exist yet, create it but don't update shares
        if len(row) != 1:
            db.execute("INSERT INTO users (username, symbol) VALUES (:id, :symbol)",
                       id=session["user_id"], symbol=symbol)

        # get previous number of shares owned
        oldshares = db.execute("SELECT shares FROM users WHERE username = :id AND symbol = :symbol",
                               id=session["user_id"], symbol=symbol)
        ...
        # update shares in users table
        db.execute("UPDATE users SET shares = :newshares WHERE username = :id AND symbol = :symbol",
        ...
        db.execute("INSERT INTO history (username, symbol, shares, method, price) VALUES (:username, :symbol, :shares, 'Buy', :price)",
                   username=session["user_id"], symbol=symbol, shares=shares, price=quote['price'])
...
def history():
    ...
    rows = db.execute("SELECT * FROM history WHERE username = :username",
...
def sell():
    ...
    if request.method == "GET":
        ...
        users = db.execute("SELECT symbol FROM users WHERE username = :id",
                           id=session["user_id"])
    ...
    # if POST method, sell stock
        ...
        shares = request.form.get("shares")
        quote = lookup(symbol)
        rows = db.execute("SELECT * FROM users WHERE username = :id AND symbol = :symbol",
                          id=session["user_id"], symbol=symbol)
        ...
        # subtract sell shares from previous shares
        newshares = oldshares - shares
        ...
        # otherwise delete stock row because no shares remain
            ...
            db.execute("DELETE FROM users WHERE symbol = :symbol AND username = :id",
                       symbol=symbol, id=session["user_id"])


        db.execute("INSERT INTO history (username, symbol, shares, method, price) VALUES (:username, :symbol, :shares, 'Sell', :price)",
                   username=session["user_id"], symbol=symbol, shares=shares, price=quote['price'])
